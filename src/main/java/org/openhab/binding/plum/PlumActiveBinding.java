...
public class PlumActiveBinding extends AbstractActiveBinding<PlumBindingProvider> implements ManagedService {
    ...
    private String sendHttpCommand(String ip, String apiCall, Map<String, Object> args) throws Exception {
        ...
        Request httppost = httpclient.POST("https://" + ip + ":8443/v2/" + apiCall);
        httppost.header("User-Agent", "Plum/2.3.0 (iPhone; iOS 9.2.1; Scale/2.00)");
        httppost.header("X-Plum-House-Access-Token", PlumUtilities.sha256(m_config.get("house_token")));
        ...
        if (response != null) {
            ...
            logger.debug("Plum HTTP Request to IP " + ip + " returned: " + status);
        ...
        for (String llid : configByLLID.keySet()) {
            ...
            for (PlumBindingConfig c : configByLLID.get(llid)) {
                ...
                if (c.getType().equals("dimmer") || c.getType().equals("switch")) {
                    int level = 0;
                    try {
                        level = jsonParsed.getInt("level");
                    } catch (JSONException e) {
                        logger.error("Error polling light level from " + c.getName() + " Plum IP " + c.getIpAddr()
                                + " LLID " + c.getLlid()
                                + ". Please verify your config file, and check that IP address and LLID values are correct");
                        continue;
                    }
    ...
    public void updated(Dictionary<String, ?> properties) throws ConfigurationException {
        ...
        if (m_config == null) {
            ...
            if (properties != null) {
                ...
                logger.warn("Plum properties was null, are you sure your config file is correct?");
            }

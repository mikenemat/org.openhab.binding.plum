    ...
    public void run() {
        ...
        while (run) {
            if (Thread.interrupted()) {
                logger.info("Plum TCP Streaming Thread for IP: " + ipAddr + " is stopping (1)...");
                break;
            }
            ...
            try {
                ...
                logger.info("Plum TCP Streaming Thread for IP: " + ipAddr + " is stopping (2)...");
                break;
            }
            ...
            try {
                ...
                String s = null;
                while ((s = input.readLine()) != null && run) {
                    if (Thread.interrupted()) {
                        logger.info("Plum TCP Streaming Thread for IP: " + ipAddr + " is stopping (3)...");
                        run = false;
                        break;
                    }
                ...
                logger.error("Plum TCP Stream error for IP " + ipAddr + " Message: " + e.getMessage());
            }
        }
        if (input != null) {
            try {
